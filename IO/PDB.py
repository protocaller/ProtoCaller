...
"""
def readPDB(id, chain_ids="all", filename="protein", process=True, **kwargs):
    if type(id) is not str:
        raise TypeError("'id' must be of type 'str'")

    id = id.replace(" ", "").upper()
    if(filename == id):
        filename = id + "_modified"
    ...
    if(process):
        _subprocess.check_call(["%s/pdb2pqr --ff=amber %s.pdb %s.pqr --ffout=amber" %
                                (_const.PDB2PQRDIR, id, filename)], shell=True)
    PDB4tleap("%s.pqr" % filename, chain_ids=chain_ids)

    if(process):
        return "%s.pqr" % filename
    else:
        return "%s.pdb" % filename

#extracts only ATOMs from a PDB and renumbers them (otherwise tleap breaks);
#see http://cupnet.net/pdb-format/ for help with PDB indices
def PDB4tleap(filename, chain_ids="all"):
    filedata = open(filename).readlines()
    output = _copy.deepcopy(filedata)
    start = None
    resnum = 0
    prev_res = ""

    for i, line in enumerate(filedata):
        if(line[:4] == "ATOM" and (chain_ids == "all" or line[21] in chain_ids)):
            if(start is None):
                start = int(line[22:26])
            if(line[22:27] != prev_res):
                prev_res = line[22:27]
                resnum += 1
            output[i] = line[:22] + "{:4d} ".format(resnum) + line[27:]
        if(line[:6] == "HETATM"):
            output[i] = ""
...
def extractSiteMolecules(filename, residue_ids=None, residue_types=None, chain_ids="all"):
    '''
    :param filename: the name of the input PDB file
    :type filename: str
    :param residue_ids: id of residues to be included in search; id is of form "L NNN" where L is the chain id and NNN
                        is the residue number; only extracts if chain id is to be included
    :type residue_ids: list of strings
    :param residue_types: types of residues to be included in search
    :type residue_types: list of a combination of the following: "waters", "ligands", "cations" and "anions"
    :param chain_ids: ids of chains to be included in search
    :type chain_ids: one of "all" or an iterable (e.g. list, string) with the chain letters which are to be included
    :return: a list of unique residue_ids
    :rtype: list of 5-character strings
    '''
    if residue_ids is None: residue_ids = []
    if residue_types is None: residue_types = ["waters", "ligands", "cations", "anions"]
    ...
    for line in file:
        if(line[:4] == "SITE"):
            found_site = True
            res_arr = [line[i:i+9] for i in range(18, len(line), 11)]
            indices += [res[4:] for res in res_arr if residueMatchesCriteria(resname=res[:3], id=res[4:],
                              residue_ids=residue_ids, residue_types=residue_types, chain_ids=chain_ids)]

    if(found_site == False):
        #print(Warning: SITE identifier in PDB file not found and thus no molecules were added. You might want to
        #         check your PDB file or manually add codes of desired molecules.)

    return list(set(indices))

#checks the type of input residue and returns True or False depending on whether it matches search criteria
def residueMatchesCriteria(resname, id, residue_ids=None, residue_types="all", chain_ids="all"):
    if((chain_ids == "all" or id[0] in chain_ids) and
       ((residue_ids is not None and id in residue_ids) or
         residue_types == "all" or
        (residue_types is not None and classifyResidue(resname) in residue_types))):
        return True
    else:
        return False

def splitHeteroAtomTypes(filename, residue_ids=None, residue_types=None, chain_ids="all"):
    if residue_ids is None: residue_ids = []
    if residue_types is None: residue_types = ["waters", "ligands", "cations", "anions"]
    ...
    for line in pdb_string:
        if line[:6] == "HETATM":
            ...
            if(residueMatchesCriteria(resname=resname, id=id, residue_ids=residue_ids, residue_types=residue_types,
                                      chain_ids=chain_ids)):
                if(classifyResidue(resname) == "water"):
                    filename = "HOH.pdb"
                else:
                    filename = "%s_%s.pdb" % (resname, id)
                filenames += [filename]
                with open(filename, "a+") as file:
                    file.write(line)
...
def findMissingResidues(filename, chain_ids="all"):
    pdb_string = open(filename).readlines()
    indices = []

    for line in pdb_string:
        match = _re.search(r"^REMARK 465\s*([\w]{3})\s*([\w])\s*([\d]*)\s*$", line)
        if match is not None:
            resname = match.group(1)
            id = "{}{:>4}".format(match.group(2), match.group(3))
            if(residueMatchesCriteria(resname=resname, id=id, chain_ids=chain_ids)):
                indices += [id]

    return list(set(indices))

def findProteinResidues(filename, chain_ids="all"):
    file = open(filename).readlines()
    indices = []
...
def renumberPDBresidues(filename, numberlist):
    filedata = open(filename).readlines()
    output = _copy.deepcopy(filedata)
    start = None
    resnum = 0
